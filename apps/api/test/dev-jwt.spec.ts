...
describe('DevJwtVerifier', () => {
  it('verifies a well-formed BUYER token and derives empty permissions', async () => {
  ...
  it('derives permissions from ADMIN role claims', async () => {
    const verifier = new DevJwtVerifier(configMock());
    const token = await issue({
      'https://parshlo.com/roles': ['ADMIN'],
    });
    const principal = await verifier.verify(token);
    expect(principal.roles).toEqual(['ADMIN']);
    expect(principal.permissions).toContain('kyc:approve');
